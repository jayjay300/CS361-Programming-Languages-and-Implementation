...
public class ConcreteSyntax {
	...
	public Program program() {
		...
		String[] header = { "void","main", "(", ")" }; //CHANGED TO FIT VOID?
	...
	private Type type() {
		...
		else if (token.getValue().equals("boolean")) //ORIGINALLY BOOL, BUT DOESN'T MATCH KEYWORDS?
			t = new Type(token.getValue());
		else
	...
	//	Identifier i;
		...
		if (token.getType().equals("Identifier")) {
			...
			a.target.id = token.getValue();//page 58 notes
			token = input.nextToken();
			match("=");
	...
	private Expression relation() {
		...
		while (token.getValue().equals("<") || token.getValue().equals("<=")
				...
				|| token.getValue().equals("==")
				|| token.getValue().equals("<>")
				|| token.getValue().equals("=")) {//equals to fix issue with !=??
			b = new Binary();
			b.term1 = e;
			b.op = new Operator(token.getValue());
			token = input.nextToken();
	...
	private Expression factor() {
		...
		if (token.getType().equals("Identifier")) {
			...
			e = v;
			token = input.nextToken();
		} else if (token.getType().equals("Literal") || token.getType().equals("Boolean")) { //fix?
			Value v = null;
	...
	//	if(token.getValue().equals("if")){//make match
			match("if");
			match("(");
			//c.test = new Expression();
			c.test = expression();
		//	token = input.nextToken();
			match(")");
